/* -*- mode: C++; coding: sjis; -*-
 * Time-stamp: <2005-05-08 21:59:26 tfuruka1>
 * $Id: dllmain.c,v 1.21 2005/05/08 13:00:47 tfuruka1 Exp $
 *
 * ak2ps のようなものの共通 DLL
 *
 * $Log: dllmain.c,v $
 * Revision 1.21  2005/05/08 13:00:47  tfuruka1
 ...
 * らない。どうやら、dll内で作成したファイルポインタを呼出し元で使用す
 * ると、アクセスヴァイオレーションが発生するようなので、使用時には要
 * 注意。
 *  *-------------------------------------------------------------------*/
FILE * WINAPI
MakeTempFile(
    ...
    )
{
    ...
    if (!(fp = MakeTempFile(mode, lpszFileName))) {
        return FALSE;
    }
    fclose(fp);
    return TRUE;
