...
Time-stamp: <2003-03-01 19:10:35 tfuruka1>


       テキストファイル印刷ツール「ak2psのようなもの」Ver. 2.1
                    ～とっても不親切なマニュアル～
...
らず、Emacsを使用していました。印刷もPostScriptプリンタを使用できる環
境にあったので、UNIXのak2psをWindows上でコンパイルして、（ごく僅かな修
正を施して）そのまま使用していました。
...
==========
    ...
    下すると、印刷の停止又は再開を行います。因みに起動時は、常に「印刷
    停止」状態になっています。私が、間違えて、無意味なファイルを大量印
    刷するのを抑止する為です(^^;。
...
印刷したいファイルをExplorerで選択し、ak2prSV.exeのメインウインドウに
ドラッグ＆ドロップして下さい。フォルダをドロップした場合は、フォルダに
含まれる全てのファイルを印刷します。
...
Usage: ak2prW [-o{p|l}] [-m{PLAIN|MAIL|PS_ACROBAT|PS_GHOST}]
    ...
    -n[-] 行番号を印字します。-n-を指定した場合は、行番号を印刷しませ
		ん。このオプションを指定しなかった場合は、サーバの設定に従いま
		す。
	-P 指定しても何もしません。
...
・ 書式の再指定(2002/4/22)
   プレビュー時に書式の再設定を出来るようにする。

・ ２ぺージ目以降のプレビュー(2002/4/22)
   ２ページ目以降のプレビューを可能にする。

・ 起動時の印刷停止状態(2002/4/22)
   起動時に常に印刷停止状態にするのではなく、最終状態を記憶しておき、
   次回起動時に復元出来るようにする。

●このドキュメントの更新履歴
============================
$Id: readme.txt,v 1.3 2003/03/01 10:11:09 tfuruka1 Exp $
$Log: readme.txt,v $
