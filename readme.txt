...
Time-stamp: <2004-08-16 23:12:10 tfuruka1>
$Id: readme.txt,v 1.7 2004/08/16 14:25:18 tfuruka1 Exp $
$Name:  $
...
==========
    ...
    全ての印刷は, ak2prSV.exe が受け持ちますので, スタートアップにショー
    ...
    ====================
        ...
        Acrobat 関連  PostScript ファイルを Acrobat Distiller を使用して
                     ...
                     Distiller の監視フォルダの[In]フォルダをフルパスで
                     指定して下さい。ak2psSv.exe は, ただたんにそのフォ
                     ルダへファイルを格納するだけです。

        GhostScript 関連 PostScript ファイルを GhostScript で印刷する
                     方は指定して下さい。GhostScript のフルパスと, そ
                     のときのオプションを指定して下さい。Default ボタ
                     ンを押下すると, デフォルトのオプションを設定しま
                     す。プレビュー後, 印刷を行い方は, GhostView を指
                     定してみても面白いと思います(私は良く使用していま
                     す)。
...
に作成できるようにしているんです。ak2pr.dll で定義している API を使用して, 
...
・ Mew で印刷したときに, ヘッダの 2行目以降が文字化けする。使用するフォ
   ントを間違えているっぽい。(2004/3/20)
...
============================
$Log: readme.txt,v $
Revision 1.7  2004/08/16 14:25:18  tfuruka1
更新履歴を追記(メール印刷でのヘッダとボディの区切り, 罫線が連結しない
問題の解決)

Revision 1.6  2004/06/18 01:08:48  tfuruka1
改行コードの修正のみです。
...
● Ver. 2.0→ Ver. 2.1
● 自分の環境が NT 4.0 から Xp に変わった(どうでも良いですけど)。
●「テキスト印刷タブ」で行番号のチェックボックスが有効になった事の記
   述を追記。
● GhostScript でプレビューを見る方法を簡単に追記
●「ak2prW.exek から自動起動」→「ak2prW.exe から自動起動」誤記訂正
